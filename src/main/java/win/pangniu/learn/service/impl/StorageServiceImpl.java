...
public class StorageServiceImpl implements StorageService {
	...
	public void uploadFileByMappedByteBuffer(MultipartFileParam param) throws IOException {
		...
		fileChannel.close();
		tempRaf.close();
		boolean isOk = this.checkAndSetUploadProgress(param, uploadDirPath);
		if (isOk) {
			boolean flag = renameFile(tmpFile, fileName);
			if (flag) {
	...
	private boolean checkAndSetUploadProgress(MultipartFileParam param, String uploadDirPath) throws IOException {
		...
		System.out.println(accessConfFile.length() + "---" + completeList.length);
		...
		if (isComplete == Byte.MAX_VALUE) {
			// stringRedisTemplate.opsForHash().put(Constants.FILE_UPLOAD_STATUS,
			// param.getMd5(), "true");
			// stringRedisTemplate.opsForValue().set(Constants.FILE_MD5_KEY +
			// param.getMd5(), uploadDirPath + "/" + fileName);
			return true;
		} else {
			// if
			// (!stringRedisTemplate.opsForHash().hasKey(Constants.FILE_UPLOAD_STATUS,
			// param.getMd5())) {
			// stringRedisTemplate.opsForHash().put(Constants.FILE_UPLOAD_STATUS,
			// param.getMd5(), "false");
			// }
			// if (stringRedisTemplate.hasKey(Constants.FILE_MD5_KEY +
			// param.getMd5())) {
			// stringRedisTemplate.opsForValue().set(Constants.FILE_MD5_KEY +
			// param.getMd5(), uploadDirPath + "/" + fileName + ".conf");
			// }
			return false;
		}
	}
	...
	public boolean renameFile(File toBeRenamed, String toFileNewName) {
		...
		Path source = Paths.get(toBeRenamed.getAbsolutePath());
		Path target = Paths.get(p + File.separator + toFileNewName);
		try {
			logger.info("File source: " + source.toAbsolutePath());
			logger.info("File target: " + target.toAbsolutePath());
			Files.move(source, target, StandardCopyOption.REPLACE_EXISTING);
			return true;
		} catch (IOException e) {
			logger.error("临时文件合并失败：" + e.getMessage());
			return false;
		}
	}
